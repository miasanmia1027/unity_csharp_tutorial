...
# 4.5 UI 기능 구현
- button 컴포넌트 안에서 클릭하면 어떤 상호작용이 되게 만들었다
- 하나의 씬에서 다른 씬으로 넘어가기 위해서는 컴포넌트만으로는 불가하다 그래서 c# 스크립트를 작성해야 한다
```
using UnityEngine;
using UnityEngine.SceneManagement; // StartScene_Manager 클래스를 사용하기 위해서

public class StartScene_Manager : MonoBehaviour
{
    // start버튼 클릭을 하면 호출
    public void OnClickStart() //-> 이거는 그냥 이름이라는 것인가
    {
        // playScene라는 이름의 씬 불러오는 역활. 즉 씬을 변경하는 역활
        SceneManager.LoadScene("2.playScene");
        // SceneManager 가 위에 클래스를 호출하여 쓰는게 가능한 클래스이다
    }
}
```
- 그리고 Ctrl + Shift + B 눌러서 씬을 등록을 해야 넘어 갈수 있다
---
#  button 컴퍼넌트가 자주 사용하는 속성
|속성|내용|
|Interactable|사용자와의 상호작용 활성화 여부|
|Transition|어떤 형태로 반응할지 선택(None/Color Tint/Sprite Swap Animation)|
|OnClick()|버튼 클릭시 호출되는 이벤트|
---